...
public class TransacaoTitulo {
	...
	public enum MotivoConfirmaçãoDeProtesto {
	...
	public MotivoConfirmaçãoDeProtesto getMotivoConfirmacaoDeProtesto(){
		Character motivo = registro.getValue("MotivoConfirmacaoDeProtesto");
		
		if(motivo != null) {
			if(motivo.equals('D')){
				return MotivoConfirmaçãoDeProtesto.DESPREZADO;
			} else if(motivo.equals('A')) {
				return MotivoConfirmaçãoDeProtesto.ACEITO;
			}
		}
		return MotivoConfirmaçãoDeProtesto.DESCONHECIDO;
	}
